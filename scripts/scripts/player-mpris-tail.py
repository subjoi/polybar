...
class Player:
    ...
    def _parseMetadata(self):
        if self._metadata != None:
            ...
            _url        = _getProperty(self._metadata, 'xesam:url', '') or _getProperty(self._metadata, 'mpris:url', '')
...
parser.add_argument('command', help="send the given command to the active player",
                    choices=[ 'play', 'pause', 'play-pause', 'stop', 'previous', 'next', 'status', 'list', 'current', 'metadata', 'raise', 'url' ],
                    default=None,
                    nargs='?')
...
parser.add_argument('--icon-stopped', default='x')
...
if args.command is None:
    ...
    elif args.commands == 'url' and current_player:
        print(_dbusValueToPython(current_player._url))
